...
function getSaoPauloDate(): { hours: number; minutes: number; dayKey: string } {
  const now = new Date();
...
function pickKeyword(dayKey: string): { keyword: string; category: string } {
...
async function postToWordPress(data: {
  ...
  imageUrl: string;
  slug: string;
}): Promise<{ wpPostId: number; wpUrl: string } | null> {
  ...
  try {
    ...
    try {
      ...
      if (cats.length > 0 && cats[0]) {
        ...
        const created = (await newCat.json()) as { id: number };
    ...
    if (!post.id) throw new Error("WordPress não retornou ID do post");
 ...
 */
async function hasTodaySoroPost(dayKey: string): Promise<boolean> {
  try {
    const rows = await db
    ...
    return rows.some((r) => {
      const brt = new Date(
        new Date(r.createdAt).toLocaleString("en-US", { timeZone: "America/Sao_Paulo" })
      );
...
export async function runSoroNow(
  forceKeyword?: string,
  forceCategory?: string
): Promise<{ slug: string; title: string; keyword: string; wpUrl?: string }> {
...
RESPONDA APENAS com um JSON válido, sem texto antes ou depois, sem backticks:
{
  ...
  if (!parsed.title || !parsed.content) throw new Error("Campos obrigatórios ausentes na resposta da IA");
...
export function startSoroScheduler(): void {
      ...
      if (alreadyPosted) {
        ...
        if (!isRunning) {
          ...
          runSoroNow()
            ...
            .catch((err: unknown) => logger.error({ err }, "Soro AI: falha no post de recuperação"))
  ...
  setInterval(() => {
    ...
    if (hours === 8 && minutes === 0 && lastPostedDate !== dayKey && !isRunning) {
      ...
      runSoroNow()
        .then(({ slug, title, wpUrl }) =>
          logger.info({ slug, title, wpUrl }, "Soro AI: publicação diária concluída")
        )
        .catch((err: unknown) => logger.error({ err }, "Soro AI: falha na publicação diária"))
        .finally(() => { isRunning = false; });
    }
  }, 60 * 1000);
}
