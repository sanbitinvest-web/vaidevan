...
interface OtpEntry { code: string; expiresAt: number; investorId: number; name: string }
...
router.post("/auth/otp/verify", async (req, res) => {
  ...
  if (!email || !code) { res.status(400).json({ error: "E-mail e código obrigatórios" }); return; }
