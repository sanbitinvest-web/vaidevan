...
import { driversTable, tripsTable, driverLocationsTable, vehiclesTable, } from "@workspace/db/schema";
import { eq, and, desc } from "drizzle-orm";
import { requireAuth, requireDriverAuth, signDriverToken, } from "../middlewares/auth";
const router = Router();
// ─── Motorista: Login ──────────────────────────────────────────────────────────
...
router.get("/driver/me", requireDriverAuth, async (req, res) => {
    try {
        const [driver] = await db
...
router.post("/driver/trip/start", requireDriverAuth, async (req, res) => {
...
router.post("/driver/trip/end", requireDriverAuth, async (req, res) => {
    try {
        const [trip] = await db
            .update(tripsTable)
            .set({ status: "ended", endedAt: new Date() })
            ...
            .returning();
        if (!trip) {
            res.status(404).json({ error: "Nenhuma viagem ativa" });
            return;
        }
        res.json({ trip });
    }
...
router.post("/driver/location", requireDriverAuth, async (req, res) => {
...
router.get("/driver/location/latest", requireDriverAuth, async (req, res) => {
    try {
        const [activeTrip] = await db
            .select()
            .from(tripsTable)
            ...
            .limit(1);
        if (!activeTrip) {
            res.json({ location: null, trip: null });
            return;
        }
...
router.get("/admin/tracking/active", requireAuth, async (_req, res) => {
    try {
        ...
        const result = await Promise.all(activeTrips.map(async (trip) => {
            const [driver] = await db
...
router.put("/admin/tracking/drivers/:id", requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (name != null)
            updates.name = name;
        if (phone != null)
            updates.phone = String(phone).replace(/\D/g, "");
        if (trackerUrl !== undefined)
            updates.trackerUrl = trackerUrl ?? null;
        if (notes !== undefined)
...
//# sourceMappingURL=driver_tracking.js.map