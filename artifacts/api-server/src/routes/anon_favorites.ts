...
function getOrCreateToken(req: Request, res: Response): string {
...
router.post("/favorites/anon", async (req, res) => {
  const token = getOrCreateToken(req, res);
  const { label, lat, lon, nickname } = req.body as {
    label?: string; lat?: string; lon?: string; nickname?: string;
  };
  if (!label || !lat || !lon) {
  ...
  try {
    const existing = await db
      ...
      .from(anonymousFavoritesTable)
      .where(and(
        eq(anonymousFavoritesTable.token, token),
        eq(anonymousFavoritesTable.label, String(label)),
      ))
      .limit(1);
...
router.delete("/favorites/anon/:label", async (req, res) => {
  ...
  try {
    await db.delete(anonymousFavoritesTable)
      .where(and(
        eq(anonymousFavoritesTable.token, token),
        eq(anonymousFavoritesTable.label, label),
      ));
    res.json({ ok: true });
  } catch (err) {
...
router.put("/favorites/anon", async (req, res) => {
  const token = getOrCreateToken(req, res);
  const items = req.body as Array<{ label: string; lat: string; lon: string; nickname?: string }>;
  ...
  try {
    await db.delete(anonymousFavoritesTable)
      .where(eq(anonymousFavoritesTable.token, token));
    if (items.length > 0) {
      await db.insert(anonymousFavoritesTable).values(
        items.map(item => ({
          token,
