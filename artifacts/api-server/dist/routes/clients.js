...
router.get("/admin/clients", requireAuth, async (_req, res) => {
    try {
...
router.get("/admin/clients/:id", requireAuth, async (req, res) => {
...
router.post("/admin/clients", requireAuth, async (req, res) => {
    const { name, email, phone, cpf, rg, cnpj, companyName, type, cep, street, number, complement, neighborhood, city, state, notes } = req.body;
    if (!name || !email)
...
router.put("/admin/clients/:id", requireAuth, async (req, res) => {
    const id = Number(req.params.id);
    const { name, email, phone, cpf, rg, cnpj, companyName, type, cep, street, number, complement, neighborhood, city, state, notes, active } = req.body;
    try {
...
// Deleta cliente
router.delete("/admin/clients/:id", requireAuth, async (req, res) => {
    const id = Number(req.params.id);
    try {
...
router.patch("/admin/clients/:id/govbr-verify", requireAuth, async (req, res) => {
    const id = Number(req.params.id);
    try {
        const [client] = await db.select().from(clientsTable).where(eq(clientsTable.id, id));
        if (!client)
