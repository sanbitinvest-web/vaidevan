...
`;
const waBtn = (phone, name) => `<a href="https://wa.me/${phone}?text=Ol%C3%A1%20${encodeURIComponent(name)}" style="display:inline-block;background:#25d366;color:#fff;padding:12px 28px;border-radius:8px;text-decoration:none;font-weight:700;font-size:14px;margin-top:20px">💬 Responder via WhatsApp</a>`;
export async function sendContactEmail(data) {
    const tp = t();
    if (!tp)
        return { sent: false, reason: "SMTP_USER/SMTP_PASS não configurados" };
...
export async function sendAdminLockoutAlert(data) {
    const tp = t();
    if (!tp)
        return { sent: false, reason: "SMTP não configurado" };
    const adminEmail = process.env.ADMIN_EMAIL || PRIMARY;
