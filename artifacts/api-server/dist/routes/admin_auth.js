...
// ── Audit log helper ──────────────────────────────────────────────────────────
async function writeAuditLog(ip, userAgent, outcome, totpVerified = false) {
    try {
...
async function getBackupCodeHashes() {
    const [row] = await db
...
async function saveBackupCodeHashes(hashes) {
...
async function consumeBackupCode(inputCode) {
...
function verifyTotp(secret, token) {
    ...
    for (const drift of [-1, 0, 1]) {
        if (computeTotp(secret, counter + drift) === token)
            return true;
    }
    return false;
}
