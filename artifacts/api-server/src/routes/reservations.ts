...
router.post("/reservations", publicFormLimiter, async (req, res) => {
  const {
    ...
    vehicleType, passengerCount, startDate, endDate,
    departureTime, returnTime, originAddress, destinationAddress,
    useAtDestination, driveAtDestination, luggageInfo, eventType,
    priority, hasBudget, budgetPhotoUrls, coastalInfo, notes,
    paymentMethod,
...
router.post("/webhooks/infinitepay", async (req, res) => {
  try {
    const { order_nsu, paid } = req.body as { order_nsu?: string; paid?: boolean; transaction_nsu?: string };
    if (order_nsu) {
...
router.patch("/admin/reservations/:id", requireAuth, async (req, res) => {
  ...
  if (paymentAmount !== undefined) updates.paymentAmount = Number(paymentAmount);
...
router.post("/admin/reservations/:id/generate-payment-link", requireAuth, async (req, res) => {
  const id = Number(req.params.id);
  const { gateway } = req.body as { gateway: "stripe" | "mercadopago" | "infinitepay" };
