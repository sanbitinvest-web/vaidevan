...
import { investorsTable, operationsTable, vehiclesTable, financialsTable, contractsTable, clientsTable, contractTemplatesTable, investmentSimulationsTable, investorFavoritesTable, } from "@workspace/db/schema";
...
router.get("/investor/dashboard", ...authAndApproved, async (req, res) => {
    const investorId = req.investorId;
    try {
        const sixMonthsAgo = new Date();
        sixMonthsAgo.setMonth(sixMonthsAgo.getMonth() - 6);
        const [[{ total: opsCount }], vehicleRows, [incomeRow, expenseRow], monthlyRows, contracts, [investor],] = await Promise.all([
            // COUNT de operações
            ...
            db.select({
                ...
                .from(financialsTable)
                .where(and(eq(financialsTable.investorId, investorId), gte(financialsTable.date, sixMonthsAgo)))
        ...
        for (const r of monthlyRows) {
            if (!monthly[r.month])
                monthly[r.month] = { income: 0, expense: 0 };
            if (r.type === "income")
                monthly[r.month].income = Number(r.total ?? 0);
            else
                monthly[r.month].expense = Number(r.total ?? 0);
        }
...
// ───── Visão Geral ─────
router.get("/investor/overview", ...authAndApproved, async (req, res) => {
    const investorId = req.investorId;
    try {
...
// ───── Financeiro ─────
router.get("/investor/financials", ...authAndApproved, async (req, res) => {
    const investorId = req.investorId;
    try {
        ...
        for (const r of records) {
            ...
            if (r.type === "income") {
                byCategory[r.category].income += Number(r.amount);
                monthly[mk].income += Number(r.amount);
            }
            else {
                byCategory[r.category].expense += Number(r.amount);
                monthly[mk].expense += Number(r.amount);
            }
        }
...
// ───── Veículos ─────
router.get("/investor/vehicles", ...authAndApproved, async (req, res) => {
    const investorId = req.investorId;
    try {
...
// ───── Contratos ─────
router.get("/investor/contracts", ...authAndApproved, async (req, res) => {
    const investorId = req.investorId;
    try {
...
router.post("/investor/contracts", ...authAndApproved, async (req, res) => {
    const investorId = req.investorId;
...
router.patch("/investor/contracts/:id/sign", ...authAndApproved, async (req, res) => {
    const investorId = req.investorId;
...
router.patch("/investor/contracts/:id/cancel", ...authAndApproved, async (req, res) => {
    const investorId = req.investorId;
    const contractId = Number(req.params.id);
    try {
        const [updated] = await db.update(contractsTable)
...
router.get("/investor/profile", requireAuth, async (req, res) => {
    const investorId = req.investorId;
    try {
        const [inv] = await db.select().from(investorsTable).where(eq(investorsTable.id, investorId));
        if (!inv)
...
router.put("/investor/profile", requireAuth, async (req, res) => {
    const investorId = req.investorId;
...
router.patch("/investor/profile/govbr-verify", requireAuth, async (req, res) => {
    const investorId = req.investorId;
    try {
        const [inv] = await db.select().from(investorsTable).where(eq(investorsTable.id, investorId));
        if (!inv)
            return res.status(404).json({ error: "Não encontrado" });
        if (!inv.cpf)
            return res.status(400).json({ error: "CPF obrigatório para vincular Gov.br" });
        const [updated] = await db.update(investorsTable)
...
router.patch("/investor/documents", requireAuth, async (req, res) => {
    const investorId = req.investorId;
    const { cnhUrl, addressProofUrl } = req.body;
    try {
        const patch = { updatedAt: new Date() };
        if (cnhUrl !== undefined) {
            patch.cnhUrl = cnhUrl || null;
            patch.cnhStatus = "pendente";
        }
        if (addressProofUrl !== undefined) {
            patch.addressProofUrl = addressProofUrl || null;
            patch.addressProofStatus = "pendente";
        }
...
router.post("/investor/simulations", requireAuth, async (req, res) => {
    const investorId = req.investorId;
    const { modality, modalityLabel, investmentAmount, months, pessimistReturn, realistReturn, optimistReturn, pessimistYield, realistYield, optimistYield, breakEvenMonthPessimist, breakEvenMonthRealist, breakEvenMonthOptimist, } = req.body;
    ...
    try {
        ...
        try {
            ...
            if (inv) {
                ...
                }).catch((e) => req.log?.warn({ err: e.message }, "Email simulação não enviado"));
            }
        }
        catch (mailErr) {
...
// ───── Endereços Favoritos ─────
router.get("/investor/favorites", requireAuth, async (req, res) => {
    const investorId = req.investorId;
    try {
...
router.post("/investor/favorites", requireAuth, async (req, res) => {
    const investorId = req.investorId;
    ...
    try {
        const existing = await db.select({ id: investorFavoritesTable.id })
            .from(investorFavoritesTable)
            .where(and(eq(investorFavoritesTable.investorId, investorId), eq(investorFavoritesTable.label, String(label))))
            .limit(1);
...
router.delete("/investor/favorites/:label", requireAuth, async (req, res) => {
    const investorId = req.investorId;
    const label = String(req.params.label);
    try {
        await db.delete(investorFavoritesTable)
            .where(and(eq(investorFavoritesTable.investorId, investorId), eq(investorFavoritesTable.label, label)));
        res.json({ ok: true });
    }
    catch {
        res.status(500).json({ error: "Erro interno" });
...
router.get("/investor/simulations", ...authAndApproved, async (req, res) => {
    const investorId = req.investorId;
    try {
