...
const autocompleteHits: RateBucket = new Map();
const directionsHits: RateBucket = new Map();

const AUTOCOMPLETE_RATE_LIMIT = 30;
const DIRECTIONS_RATE_LIMIT = 5;

function checkRateLimitBucket(bucket: RateBucket, limit: number, ip: string): boolean {
  const now = Date.now();
...
router.get("/maps/autocomplete", async (req: Request, res: Response) => {
  ...
  const { q } = req.query;
  if (!isValidAddress(q as string) || (q as string).trim().length < 3) {
    res.status(400).json({ error: "q must be at least 3 characters" });
    return;
  }

  try {
    const url = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(q as string)}&format=json&limit=5&countrycodes=br&addressdetails=0`;
    ...
    const data = await nominatimRes.json() as Array<{ display_name: string; lat: string; lon: string }>;
  ...
  try {
    ...
    if (osrmData.code !== "Ok" || !osrmData.routes?.length) {
      ...
      if (validCoords.length >= 3) {
        let failedStopIndex: number | undefined;
        for (let i = 0; i < validCoords.length - 1; i++) {
          ...
          try {
            ...
            if (legRes.ok) {
              const legData = await legRes.json() as { code: string };
              if (legData.code !== "Ok") {
                failedStopIndex = i + 1;
                break;
              }
...
const tileCache = new Map<string, { buf: Buffer; ct: string; ts: number }>();
const TILE_CACHE_TTL = 1000 * 60 * 60 * 24; // 24 h

router.get("/maps/tiles/:z/:x/:y", async (req: Request, res: Response) => {
