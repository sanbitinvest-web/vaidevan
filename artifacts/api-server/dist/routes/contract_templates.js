...
const router = Router();
// Lista templates
router.get("/admin/contract-templates", requireAuth, async (_req, res) => {
    try {
...
router.get("/admin/contract-templates/:id", requireAuth, async (req, res) => {
...
// Cria template
router.post("/admin/contract-templates", requireAuth, async (req, res) => {
    const { name, type, description, body, fields } = req.body;
    if (!name || !body)
        return res.status(400).json({ error: "Nome e corpo são obrigatórios" });
    try {
...
// Atualiza template
router.put("/admin/contract-templates/:id", requireAuth, async (req, res) => {
    const id = Number(req.params.id);
...
// Deleta template
router.delete("/admin/contract-templates/:id", requireAuth, async (req, res) => {
    const id = Number(req.params.id);
    try {
...
//# sourceMappingURL=contract_templates.js.map