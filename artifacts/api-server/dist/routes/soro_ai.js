...
router.post("/soro/generate", requireAuth, async (req, res) => {
    const { keyword, category, customTitle } = req.body;
    if (!keyword) {
    ...
    try {
        ...
        while (true) {
            ...
            if (!contentType.includes("json")) {
                ...
                throw new Error(`WordPress returned an unexpected response (content-type: ${contentType}). ` +
        ...
        for (const wpPost of allWpPosts) {
            ...
            const wpSlug = wpPost.slug ?? slugify(title);
            ...
            for (const termList of embeddedTerms) {
                ...
                if (category !== "Transporte Executivo")
                    break;
            }
            // Extrair imagem
            let image = "";
...
router.post("/soro/batch-generate", requireAuth, async (req, res) => {
    const { count = 10, keywords: customKeywords } = req.body;
    ...
    (async () => {
        const kws = Array.isArray(customKeywords) && customKeywords.length
            ? customKeywords
...
//# sourceMappingURL=soro_ai.js.map