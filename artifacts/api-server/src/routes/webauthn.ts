...
function setChallenge(key: string, challenge: string) {
  ...
  for (const [k, v] of challenges) {
    if (Date.now() > v.expires) challenges.delete(k);
  }
}
...
function getRpId(req: Request): string {
  if (process.env.NODE_ENV === "production") return "vaidevan.com";
...
function getExpectedOrigins(req: Request): string[] {
  ...
  const devDomain = process.env.REPLIT_DEV_DOMAIN;
  if (devDomain && !origins.includes(`https://${devDomain}`)) origins.push(`https://${devDomain}`);
  if (origins.length === 0) origins.push("http://localhost:3000", "http://localhost:5173");
  return origins;
}

// ── Portal (Investor) — Registration ───────────────────────────────────────

router.post("/webauthn/register/options", requireAuth, async (req: AuthRequest, res) => {
  try {
    const [investor] = await db.select().from(investorsTable).where(eq(investorsTable.id, req.investorId!));
    if (!investor) { res.status(404).json({ error: "Usuário não encontrado." }); return; }
...
router.post("/webauthn/register/verify", requireAuth, async (req: AuthRequest, res) => {
  try {
    const [investor] = await db.select().from(investorsTable).where(eq(investorsTable.id, req.investorId!));
    if (!investor) { res.status(404).json({ error: "Usuário não encontrado." }); return; }
    ...
    if (!expectedChallenge) { res.status(400).json({ error: "Sessão de cadastro expirada. Tente novamente." }); return; }
...
router.post("/webauthn/auth/verify", async (req, res) => {
  try {
    ...
    if (!investor || !investor.active) { res.status(401).json({ error: "Usuário não encontrado ou inativo." }); return; }

    const expectedChallenge = getAndDeleteChallenge(`inv_auth_${email}`);
    if (!expectedChallenge) { res.status(400).json({ error: "Sessão expirada. Tente novamente." }); return; }

    const userId = String(investor.id);
    const credentialIdFromResponse = req.body.id as string;

    const [storedCred] = await db.select().from(webauthnCredentialsTable)
      .where(and(
        eq(webauthnCredentialsTable.credentialId, credentialIdFromResponse),
        eq(webauthnCredentialsTable.userId, userId),
        eq(webauthnCredentialsTable.userType, "investor"),
      ));

    if (!storedCred) { res.status(401).json({ error: "Credencial biométrica não encontrada." }); return; }
    ...
    const verification = await verifyAuthenticationResponse({
      ...
      credential: {
        ...
        transports: JSON.parse(storedCred.transports ?? "[]") as AuthenticatorTransportFuture[],
      },
    });

    if (!verification.verified) { res.status(401).json({ error: "Autenticação biométrica falhou." }); return; }
...
router.get("/webauthn/credentials", requireAuth, async (req: AuthRequest, res) => {
  try {
...
router.delete("/webauthn/credentials/:id", requireAuth, async (req: AuthRequest, res) => {
  try {
    const credId = Number(req.params.id);
    const userId = String(req.investorId!);
    await db.delete(webauthnCredentialsTable).where(
      and(eq(webauthnCredentialsTable.id, credId), eq(webauthnCredentialsTable.userId, userId))
    );
...
router.post("/webauthn/public/register/options", async (req, res) => {
  try {
    const { email, name, userType } = req.body as { email?: string; name?: string; userType?: string };
    if (!email || !userType) { res.status(400).json({ error: "E-mail e tipo de usuário obrigatórios." }); return; }
    if (!["partner", "client"].includes(userType)) { res.status(400).json({ error: "Tipo inválido." }); return; }
...
router.post("/webauthn/public/register/verify", async (req, res) => {
  try {
    const { email, name, userType, registrationResponse } = req.body as {
      email?: string; name?: string; userType?: string;
...
router.get("/webauthn/public/status", async (req, res) => {
  try {
    const { email, userType } = req.query as { email?: string; userType?: string };
    if (!email || !userType) { res.status(400).json({ error: "E-mail e tipo obrigatórios." }); return; }
    const userId = `${userType}_${email.toLowerCase().trim()}`;
