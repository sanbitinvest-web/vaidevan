...
import { requireAdminOrApprovedAuth, requireAdminAuth, AuthRequest } from "../middlewares/auth";
...
router.get("/admin/investor-applications/:id", requireAdminOrApprovedAuth, async (req, res) => {
  ...
  try {
    const [inv] = await db
      ...
      .limit(1);
    if (!inv) { res.status(404).json({ error: "Candidato não encontrado." }); return; }
...
router.patch("/admin/investor-applications/:id/review", requireAdminAuth, async (req: AuthRequest, res) => {
  const id = Number(req.params.id);
  try {
    ...
    await db
      ...
      .where(eq(partnersTable.id,
        (await db.select({ pid: investorsTable.partnerApplicationId })
...
router.patch("/admin/investor-applications/:id/approve", requireAdminAuth, async (req: AuthRequest, res) => {
  const id = Number(req.params.id);
  ...
  try {
    ...
    if (kycNotes !== undefined) patch.kycNotes = kycNotes;
...
router.patch("/admin/investor-applications/:id/reject", requireAdminAuth, async (req: AuthRequest, res) => {
  const id = Number(req.params.id);
  ...
  try {
    ...
    if (rejectedReason !== undefined) patch.rejectedReason = rejectedReason;
    if (kycNotes !== undefined) patch.kycNotes = kycNotes;
...
router.patch("/admin/investor-applications/:id/notes", requireAdminAuth, async (req: AuthRequest, res) => {
  const id = Number(req.params.id);
...
router.patch("/admin/investor-applications/:id/reset-password", requireAdminAuth, async (req: AuthRequest, res) => {
  const id = Number(req.params.id);
