...
// ── Admin: criar veículo ──────────────────────────────────────────────────────
router.post("/admin/vehicle-listings", requireAuth, async (req, res) => {
    const { brand, model, year, color, plate, fuelType, transmission, mileage, passengerCapacity, cargoCapacity, enginePower, accessories, benefits, photos, coverPhoto, price, priceNegotiable, expectedReturnMonths, monthlyReturn, monthlyReturnPercent, description, condition, location, status, featured, sortOrder, } = req.body;
...
router.put("/admin/vehicle-listings/:id", requireAuth, async (req, res) => {
    const id = Number(req.params.id);
    const { brand, model, year, color, plate, fuelType, transmission, mileage, passengerCapacity, cargoCapacity, enginePower, accessories, benefits, photos, coverPhoto, price, priceNegotiable, expectedReturnMonths, monthlyReturn, monthlyReturnPercent, description, condition, location, status, featured, sortOrder, } = req.body;
    try {
        ...
        if (year !== undefined)
            patch.year = Number(year);
        if (color !== undefined)
            patch.color = color.trim();
        if (plate !== undefined)
            patch.plate = plate?.trim() || null;
        if (fuelType !== undefined)
            patch.fuelType = fuelType;
        if (transmission !== undefined)
            patch.transmission = transmission;
        if (mileage !== undefined)
            patch.mileage = Number(mileage);
        if (passengerCapacity !== undefined)
            patch.passengerCapacity = passengerCapacity ? Number(passengerCapacity) : null;
        if (cargoCapacity !== undefined)
            patch.cargoCapacity = cargoCapacity?.trim() || null;
        if (enginePower !== undefined)
            patch.enginePower = enginePower?.trim() || null;
        if (accessories !== undefined)
            patch.accessories = Array.isArray(accessories) ? accessories : [];
        if (benefits !== undefined)
            patch.benefits = Array.isArray(benefits) ? benefits : [];
        if (photos !== undefined)
            patch.photos = Array.isArray(photos) ? photos : [];
        if (coverPhoto !== undefined)
            patch.coverPhoto = coverPhoto?.trim() || null;
        if (price !== undefined)
            patch.price = price ? Number(price) : null;
        if (priceNegotiable !== undefined)
            patch.priceNegotiable = priceNegotiable;
        if (expectedReturnMonths !== undefined)
            patch.expectedReturnMonths = expectedReturnMonths ? Number(expectedReturnMonths) : null;
        if (monthlyReturn !== undefined)
            patch.monthlyReturn = monthlyReturn ? Number(monthlyReturn) : null;
        if (monthlyReturnPercent !== undefined)
            patch.monthlyReturnPercent = monthlyReturnPercent ? Number(monthlyReturnPercent) : null;
...
router.patch("/admin/vehicle-listings/:id/status", requireAuth, async (req, res) => {
    const id = Number(req.params.id);
...
router.delete("/admin/vehicle-listings/:id", requireAuth, async (req, res) => {
    const id = Number(req.params.id);
    try {
...
//# sourceMappingURL=vehicle_listings.js.map